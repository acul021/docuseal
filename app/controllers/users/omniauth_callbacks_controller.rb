...
class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  def oidc
    ...
    unless account
      return redirect_to new_user_session_path, alert: I18n.t('sso_configuration_not_found')
    end

    email  = auth.info.email.to_s.strip.downcase
    config = oauth_config_for(account)

    if !email_domain_allowed?(email, config)
    ...
    unless user
      return redirect_to new_user_session_path, alert: I18n.t('sso_user_not_found')
    end
